<div class="relative">
   ...
   <div class="relative" x-data="{ 
    ...
    async fetchSuppliers() {
        ...
        const response = await fetch(`/crm/api/supplier?search=${encodeURIComponent(this.search)}`);
    ...
    <label for="supplier_search" class="block text-sm font-medium text-gray-700">Customer</label>
    ...
    <div class="relative mt-1">
        ...
        <input 
            type="text" 
            x-model="search"
            @input.debounce.500ms="fetchSuppliers()"
        ...
        <input type="hidden" name="supplier_id" x-model="selectedId">
    </div>
    ...
    <div x-show="showDropdown && search.length >= 2" 
        ...
        <template x-if="suppliers.length > 0">
            <ul>
                <template x-for="customer in suppliers" :key="customer.id">
        ...
        <template x-if="suppliers.length === 0">
            <div class="text-gray-500 py-2 pl-3">No suppliers found</div>
        </template>
    </div>
</div>
</div>