        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
            ...
            <div>
                ...
                <div class="relative" x-data="{ show:false }">
                    <input :type="show ? 'text': 'password' " id="password" class="w-full px-4 py-3 border border-[#E5E7EB] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4DA8DA]/80 focus:border-transparent transition placeholder:text-gray-400"
                                  type="password"
                                  name="password"
                                  required
                                  autocomplete="current-password"
                                  placeholder="••••••••" />
                    <button @click="show = !show" id="passwordToggle" type="button" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-[#999] hover:text-[#333] transition">
